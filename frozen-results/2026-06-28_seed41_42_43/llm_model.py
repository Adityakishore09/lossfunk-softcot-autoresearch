...
def hf_token() -> Optional[str]:
    """Use an explicitly configured token when required; public models need none."""
...
class EfficientSoftCoTFromSmallModel(SoftCoTAbstractClass):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if soft_thought_control not in ['learned', 'zero']:
            raise ValueError(
                f'Unsupported soft-thought control: {soft_thought_control}'
            )
        self.soft_thought_control = soft_thought_control
    ...
    def get_inputs_embeds_for_base_model(
        ...
        if self.soft_thought_control == 'zero':
            projected_inputs_embeds = torch.zeros_like(projected_inputs_embeds)
