...
if task_name in ['gsm8k']:
    if data_dir is None:
        ...
        db = GSM8KLoader().load({
            'train': os.path.join(data_dir, 'train_socratic.jsonl'),
            'dev': os.path.join(data_dir, 'test_socratic.jsonl'),
        })
...
training_args = TrainingArguments(
    ...
    eval_strategy='epoch',
    # Trainer's generic safetensors checkpointing serializes the frozen tied
    # assistant-model embedding/output weights and fails on this model class.
    # The experiment needs only the final projection, saved explicitly below.
    save_strategy='no',
    learning_rate=2e-5,
